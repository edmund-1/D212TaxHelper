...
| 7 | ~~**No D212 XML export.**~~ Skeleton-level export shipped: cap11 + cap14 elements with all computed values; personal data placeholders the user fills in the ANAF tool. `lib/d212-xml-builder.js` + POST `/api/d212-xml/:year` + button on Calcul Impozite tab. Full multi-country (D-3) and schematron re-validation still open. | High — UX | High | ✅ DONE — skeleton (commit, this one). Remaining work: D-3 multi-country wiring, schematron pre-flight. |
